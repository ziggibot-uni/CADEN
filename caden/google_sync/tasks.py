...
class TasksClient:
    def __init__(
        ...
        *,
        readable_task_list_ids: tuple[str, ...] | None = None,
        writable_task_list_id: str | None = None,
    ) -> None:
        try:
        ...
        self.readable_task_list_ids = readable_task_list_ids or (task_list_id,)
        self.writable_task_list_id = writable_task_list_id or task_list_id
        self.task_list_id = self.writable_task_list_id

    def create(self, title: str, due: datetime | None = None, notes: str = "") -> GTask:
        body = {
            "title": title,
            "notes": notes,
        }
        if due is not None:
            ...
            body["due"] = due_anchor.strftime("%Y-%m-%dT%H:%M:%S.000Z")
        try:
            t = self.service.tasks().insert(tasklist=self.writable_task_list_id, body=body).execute()
    ...
    def list_open(self) -> list[GTask]:
        out: list[GTask] = []
        for task_list_id in self.readable_task_list_ids:
            try:
                resp = (
                    self.service.tasks()
                    .list(tasklist=task_list_id, showCompleted=False, maxResults=100)
                    .execute()
                )
            except HttpError as e:
                raise GoogleSyncError(f"Tasks list failed: {e}") from e
            out.extend(_to_task(t) for t in resp.get("items", []))
        return out

    def get(self, task_id: str) -> GTask:
        try:
            t = self.service.tasks().get(tasklist=self.writable_task_list_id, task=task_id).execute()
        except HttpError as e:
    ...
    def mark_completed(self, task_id: str) -> None:
        try:
            self.service.tasks().patch(
                tasklist=self.writable_task_list_id,
                task=task_id,
